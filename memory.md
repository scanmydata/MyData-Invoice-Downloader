...
**ΜΗΝ ΞΑΝΑΒΑΛΕΙΣ «επαλήθευση» που σβήνει το task (λάθος 0.2.28, αναιρέθηκε στην
0.2.30):** μπήκε λογική που περίμενε ένα start-token στο run_log για λίγα δευτ.
και, αν δεν εμφανιζόταν, έσβηνε το scheduled task και πήγαινε σε εφεδρεία. Αυτό
βασίστηκε σε ΛΑΘΟΣ διάγνωση: σε sandbox/μη-διαδραστικό session (π.χ. το shell
των εργαλείων) τα detached processes ΔΕΝ τρέχουν και το task μένει *Queued* για
πάντα — αλλά στο ΠΡΑΓΜΑΤΙΚΟ διαδραστικό session του χρήστη το `schtasks` δούλευε
μια χαρά επί μέρες. Η «επαλήθευση» με μικρό timeout (6s) + σβήσιμο ακύρωνε ένα
task που απλώς αργούσε να ξεκινήσει (π.χ. όσο ο Defender σαρώνει το φρέσκο,
ανυπόγραφο setup.exe). Αποτέλεσμα: «η ενημέρωση δεν ξεκίνησε». **Σωστό:**
εμπιστεύσου το `schtasks` — αν καταχωρηθεί+πυροδοτηθεί, γύρνα `True`, ΚΛΕΙΣΕ την
εφαρμογή, και το task τρέχει όποτε είναι έτοιμο. Μην κρατάς την εφαρμογή ανοιχτή
να «περιμένει». Το μόνο που κρατήθηκε: ο έλεγχος `/TR` > **261 χαρακτήρες** →
κατευθείαν εφεδρεία.
