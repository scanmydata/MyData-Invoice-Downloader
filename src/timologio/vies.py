...
Η ιδέα και ο καθαρισμός του «||» είναι από το ScanmyData_private/vat_validator.py.
Δύο διαφορές:
...
def clean_vat(vat: str) -> str | None:
    """Καθαρίζει ΑΦΜ σε 9 ψηφία (vat_validator.py:139-168)."""
    if not vat:
        return None
...
def clean_name(name: str | None) -> str | None:
    ...
    και το vat_validator.py:170-182. Το «---» σημαίνει «δεν δίνεται όνομα».
    """
    if not name:
        return None
    text = str(name)
